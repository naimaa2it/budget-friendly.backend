...
async function main() {
  const URI = process.env.MONGODB_URI || process.env.MONGO_URI || 'mongodb://localhost:27017/yourhaat';
  ...
  if (allProducts.length === 0) {
    console.log('No products found — add products first, then re-run this script.');
    await mongoose.disconnect();
    return;
  }

  // ── Helper: filter by badge ───────────────────────────────────────────────
  const byBadge = (badge) => allProducts.filter(p => (p.badges || []).includes(badge));
  ...
  const fashion = [
    ...
    ...byCatName('jewelry'),
    ...byCatName('jewel'),
  ].filter((p, i, a) => a.findIndex(x => x._id.equals(p._id)) === i).slice(0, 10);
  if (fashion.length > 0) {
    sectionsToCreate.push({
  ...
  if (sectionsToCreate.length === 0) {
    ...
    console.log('No badge/category matches found — created a generic Featured Products section.');
  }
  ...
  for (const sec of sectionsToCreate) {
    ...
    await s.save();
    console.log(`✅ Created: "${s.title}"  (${sec.productIds.length} products)`);
  }
