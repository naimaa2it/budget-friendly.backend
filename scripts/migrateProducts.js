...
async function run() {
  ...
  for (const doc of arraySpecDocs) {
    ...
    for (const spec of original) {
      ...
      if (spec.type === "header" || spec.type === "row") {
        ...
        if (spec.type === "header") {
          ...
          if (typeof spec.value === "object" && spec.value !== null) {
            ...
            changed = true;
            console.log(
              `    ⚠ dropped corrupt row (object value) in "${doc.title}"`,
            );
          } else {
            cleaned.push({
              type: "row",
      ...
      if (hasKey || hasValue) {
        if (typeof spec.value === "object" && spec.value !== null) {
          ...
          changed = true;
          console.log(
            `    ⚠ dropped corrupt row (no type, object value) in "${doc.title}"`,
          );
          continue;
        }
        cleaned.push({
    ...
    if (changed) {
      ...
      cleanedCount++;
      console.log(
        `  ✓ [${doc._id}] "${doc.title}" — ${original.length} → ${cleaned.length} specs`,
      );
    }
  }
  ...
  for (const doc of weirdDescDocs) {
    console.log(
      `  • [${doc._id}] "${doc.title}" — clearing detailedDescription`,
    );
    await col.updateOne(
      { _id: doc._id },
      { $unset: { detailedDescription: "" } },
    );
  }
