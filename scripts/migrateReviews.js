...
async function run() {
    ...
    if (product.reviews && product.reviews.length > 0) {
      ...
      if (existingCount === 0) {
        ...
        try {
          ...
          console.error(`Error migrating reviews for product ${productId}:`, err.message);
          errors++;
        }
      }
    }
    ...
    if (product.faqs && product.faqs.length > 0) {
      ...
      if (existingCount === 0) {
        const docs = product.faqs.map((f, i) => ({
          ...
          answeredBy: f.answeredBy || 'YourHaat Team',
        ...
        try {
          ...
          console.error(`Error migrating FAQs for product ${productId}:`, err.message);
          errors++;
        }
      }
    }
  ...
  console.log('');
  console.log('Next step: after verifying the data, remove the reviews/faqs arrays');
  console.log('from the Product schema and drop the embedded fields from the collection.');
