...
function trimCredentialObject(creds) {
...
function mergeEnvCredentials(slug, creds) {
  const envCreds = envCredentialsForSlug(slug);
  if (!envCreds) return { creds, envApplied: false };
  const trimmedEnv = trimCredentialObject(envCreds);
  const merged = { ...(creds || {}) };
  let envApplied = false;
  for (const [key, value] of Object.entries(trimmedEnv)) {
    if (value !== '' && value != null) {
      merged[key] = value;
      envApplied = true;
    }
  }
  return { creds: merged, envApplied };
}
...
function envCredentialsForSlug(slug) {
  ...
  if (s === 'steadfast') {
    const creds = {
      ...
      baseUrl: process.env.STEADFAST_BASE_URL || STEADFAST_API_BASE,
    };
...
export async function getCourierIntegration(slug) {
  ...
  if (doc?.credentialsEncrypted) {
    creds = trimCredentialObject(decryptJson(doc.credentialsEncrypted));
    if (Object.keys(creds).length) source = 'database';
  }

  const envMerge = mergeEnvCredentials(slug, creds);
  creds = envMerge.creds;
  if (envMerge.envApplied) {
    source = source === 'database' ? 'env+database' : source === 'none' ? 'env' : source;
  } else if (!credentialsConfigured(slug, creds, storeConfig)) {
    const envOnly = envCredentialsForSlug(slug);
    if (envOnly) {
      creds = trimCredentialObject(envOnly);
      if (credentialsConfigured(slug, creds, storeConfig)) source = 'env';
    }
  }

  if (String(slug).toLowerCase() === 'steadfast' && creds) {
    creds.baseUrl = STEADFAST_API_BASE;
  }
