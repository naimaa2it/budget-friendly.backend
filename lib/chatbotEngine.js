...
// "yes / no" the customer might type at a "want to see more?" prompt. Kept as
// dedicated, unambiguous sets (NOT KW.yes/KW.cancel) so a bare product name
// isn't mistaken for a pagination answer.
const MORE_YES = [
  "yes","haa","ha","hae","hyaa","hmm","aro","aro dekhabo","aro dekhao","aro dekhte chai",
  "show more","more","dekhao","dekhan","next",
  "হ্যাঁ","হ্যা","হা","আরও","আরো","আরও দেখান","আরও দেখতে চাই","আরও দেখাও","দেখাও","দেখান","জি",
];
const MORE_NO = [
  "no","na","nah","nope","enough","byas","bas","ar na","ar lagbe na",
  "না","নাহ","আর না","ব্যাস","যথেষ্ট","হবে না",
];
...
async function _handle({ text, state, meta = {} }) {
  ...
  if (step === "choose_product") {
    ...
    // At a "want to see more?" prompt: "হ্যাঁ" → next page, "না" → stop.
    if (s.hasMore && s.page) {
      if (saysAny(text, MORE_YES)) {
