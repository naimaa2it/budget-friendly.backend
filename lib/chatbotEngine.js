...
// Shorten a marketing-heavy product title to its core name — drop everything
// after the first "|" separator (the "| Pop-Up Camera | Bluetooth Calling"
// tail) and cap the length — so chat product lists stay clean instead of each
// item wrapping across three or four lines.
function cleanTitle(t) {
  let s = String(t || "").split("|")[0].replace(/\s+/g, " ").trim();
  if (s.length > 46) s = s.slice(0, 45).trim() + "…";
  return s;
}

// Render a numbered product list — one product over two tidy lines (name, then
// an indented price) with a blank line between items, so it reads cleanly in a
// chat bubble. `items` is an array of { title, price }. `start` continues the
// numbering across "show more" pages (page 2 begins at 9, 10, …).
function listLines(items, start = 0) {
  return items
    .map((it, i) => `${start + i + 1}. ${cleanTitle(it.title)}\n    💵 ${money(it.price)}`)
    .join("\n\n");
}
...
const SAY = {
  ...
  // Asked when a customer says "I want to see products" WITHOUT naming one —
  // we ask which product they want instead of dumping the whole catalog.
  askProduct: [
    "অবশ্যই 🙂 কোন পণ্যটি দেখতে চান? পণ্যের নাম লিখুন — যেমন ঘড়ি, ইয়ারফোন, রাউটার…",
    "জি 🙂 কোন পণ্যটা খুঁজছেন? নামটা লিখুন, সাথে সাথে দেখিয়ে দিচ্ছি 🙂",
    "আচ্ছা 🙂 কী ধরনের পণ্য দেখতে চান? পণ্যের নাম বা ধরনটা লিখুন (যেমন: হেডফোন, চার্জার, স্মার্ট ওয়াচ…)।",
    "নিশ্চয়ই 🙂 বলুন তো কোন পণ্যটা দেখতে চান — নামটা লিখলেই লিস্ট দিয়ে দিচ্ছি।",
  ],
};

// Canonical (post-stem) tokens that make up a bare "show me products" phrase.
// When a browse message contains ONLY these, we ask which product; a real
// product word beyond them (e.g. "router") skips the question and searches.
const BROWSE_WORDS = new Set([
  "product", "products", "item", "items", "list", "catalog", "browse",
  "sob", "all", "kichu", "available", "stock", "gula", "gulo", "guli",
  "dekhte", "dekhbo", "dekhabo", "dekhi", "dekhben", "dekhao", "dekhan", "dekha",
  "koto", "paoa", "jay",
  // Bangla-script equivalents (post-stem forms of the browse phrases)
  "দেখব", "দেখবো", "দেখতে", "দেখাও", "দেখান", "দেখি", "সব", "তালিকা",
  "গুলো", "গুলি", "গুলা", "কিছু",
]);
...
const SYN = {
  ...
  // ── Banglish spelling variants & common misspellings → canonical token ──
  // Customers type the same word a dozen ways in Latin script ("delivary",
  // "delibhari", "charj", "warenty"). Folding them here means QA-matching and
  // product search treat them all the same as the clean English token — this is
  // what makes the bot forgiving of real-world Banglish typing.
  "delivary": "delivery", "delibhari": "delivery", "delibary": "delivery",
  "delibery": "delivery", "dilivary": "delivery", "dilibhari": "delivery",
  "deliveri": "delivery", "delevery": "delivery",
  "charj": "charge", "charz": "charge", "khoroch": "charge", "khorch": "charge",
  "warenty": "warranty", "warrenty": "warranty", "warent": "warranty",
  "warenti": "warranty", "garanti": "warranty", "gyaranti": "warranty",
  "gyarantee": "warranty", "warrentee": "warranty", "waranty": "warranty",
  "daam": "price", "dam": "price", "mullo": "price", "mulyo": "price",
  "damkoto": "price", "rate": "price",
  "ordar": "order", "ordr": "order", "oder": "order", "ordear": "order",
  "ferot": "return", "ferat": "return", "fert": "return",
  "rifund": "refund", "refnd": "refund",
  "exchenge": "exchange", "exchang": "exchange", "ekschenj": "exchange",
  "poyment": "payment", "peyment": "payment", "payemnt": "payment",
  "bikash": "bkash", "bekash": "bkash", "bkas": "bkash",
  "nagod": "nagad", "nogod": "nagad",
  "roket": "rocket",
  "nombor": "number", "nombr": "number", "nmbr": "number",
  "stok": "stock", "stoke": "stock",
  "discount": "discount", "discunt": "discount", "chhar": "discount",
  "chhaar": "discount", "kome": "komano", "komabe": "komano", "komben": "komano",
  "asol": "original", "oiginal": "original", "originl": "original",
  "kualiti": "quality", "koaliti": "quality",
  "kalar": "color", "colour": "color", "rong": "color",
  "chobi": "image", "picture": "image", "pic": "image",
  "spec": "specification", "specs": "specification", "specification": "specification",
  "fitcher": "feature", "features": "feature",
  "gift": "gift", "upohar": "gift", "free": "free", "fri": "free",
  "kobe": "kobe", "koydin": "din", "kotodin": "din", "kotdin": "din",
  "installment": "installment", "kisti": "installment", "emi": "emi",
  "sthan": "location", "location": "location", "thikana": "address",
  "review": "review", "rivew": "review", "man": "quality",
  // product-name Banglish variants → English product-title words
  "ghori": "watch", "watch": "watch",
  "hedphone": "headphone", "headfone": "headphone", "hedfone": "headphone",
  "iarfon": "earphone", "earfone": "earphone",
  "iarbud": "earbud", "earbuds": "earbud",
  "charjar": "charger", "charzar": "charger",
  "spiker": "speaker", "fan": "fan", "faan": "fan",
  "batteri": "battery", "betari": "battery",
  "rauter": "router", "wifi": "router", "wai-fi": "router",
  "maus": "mouse", "kiboard": "keyboard", "keybord": "keyboard",
  "nekband": "neckband", "nekbband": "neckband", "neckbend": "neckband",
  "mobail": "mobile", "phone": "phone", "fon": "phone",
  "laptp": "laptop", "kebol": "cable", "keble": "cable",
  "trimar": "trimmer", "trimmar": "trimmer", "traimar": "trimmer",
  "lait": "light", "gejet": "gadget", "gadjet": "gadget",
};
...
// Like has(), but single-word triggers must match a WHOLE word (multi-word
// phrases still match as a substring). Used for pagination yes/no, where a
// loose substring on short tokens like "ha"/"no" would wrongly fire on product
// names ("charger" contains "ha", "nokia" contains "no").
function saysAny(text, list) {
  const n = normalize(text);
  const words = new Set(n.split(/\s+/));
  return list.some((k) => (k.includes(" ") ? n.includes(k) : words.has(k)));
}
...
const KW = {
  ...
  // NOTE: yes/no now live in the shared YES / NO sets (see isYes / isNo).
  track: [
...
const SMALLTALK = [
  ...
  {
    // "how do I order?" — the single most common process question, in all 3 scripts.
    k: ["kivabe order korbo","kmne order korbo","order kmne","how to order","how do i order",
        "order kivabe","order kibhabe dibo","order kbe korbo","kivabe kinbo","kibhabe kinbo",
        "কিভাবে অর্ডার","কীভাবে অর্ডার","কেমনে অর্ডার","কিভাবে কিনব","কীভাবে কিনব","অর্ডার কিভাবে"],
    a: [
      'খুব সহজ 🙂 শুধু পণ্যের নামটা এখানে লিখুন — আমি দাম দেখিয়ে "অর্ডার" নেওয়া শুরু করব। এরপর নাম, নাম্বার আর ঠিকানা দিলেই ক্যাশ অন ডেলিভারিতে অর্ডার কনফার্ম 🥰',
    ],
  },
  {
    // cash-on-delivery availability — asked constantly before ordering.
    k: ["cash on delivery","cod ache","cod hobe","hate peye taka","product hate peye","hate pa oyar por",
        "advance lagbe","agdam lagbe","cash on","ক্যাশ অন ডেলিভারি","হাতে পেয়ে টাকা","পণ্য হাতে পেয়ে",
        "অগ্রিম লাগবে","আগাম লাগবে","টাকা আগে"],
    a: [
      "জি, ক্যাশ অন ডেলিভারি আছে ✅ পণ্য হাতে পেয়ে টাকা দিতে পারবেন 🙂 (কিছু ক্ষেত্রে সামান্য অগ্রিম লাগতে পারে — পণ্যের নাম লিখুন, বিস্তারিত জানিয়ে দিচ্ছি।)",
    ],
  },
  {
    // price haggling / "is the price fixed?" — very common in BD.
    k: ["dam koto komabe","dam komano jabe","kom hobe na","dordam","fixed price","dam fixed","last koto",
        "একটু কমান","দাম কমানো যাবে","দাম কি ফিক্সড","দরদাম","লাস্ট কত","কম হবে না"],
    a: [
      "দামটা প্রায় ফিক্সড রাখি 🙂 তবে অফার/ডিসকাউন্ট চললে সেটা এমনিতেই দামে বসানো থাকে। কোন পণ্যটা নিয়ে বলছেন? — দেখে বেস্ট প্রাইসটা জানিয়ে দিচ্ছি।",
    ],
  },
];
...
// ── product-list pager ("show more") ─────────────────────────────────────────
// We show products a page at a time and ask "want to see more?" instead of
// dumping everything. The choose_product state carries a `page` descriptor so
// "yes" can fetch the next batch.
const PAGE = 8;

// Every "yes" / "no" a customer might type — English + Banglish + Bangla — so
// confirmations and the "show more?" prompt accept them all. Matched with
// saysAny() (whole word for single tokens), so short forms like "ha"/"na"/"ok"
// don't falsely fire inside product names ("charger" contains "ha").
const YES = [
  // English
  "yes","yeah","yep","yup","ya","yah","y","ok","okay","okey","okk","okey","k","kk",
  "sure","fine","alright","ryt","right","correct","confirm","confirmed","done","agree",
  "agreed","accept","proceed","yess","yesss",
  // Banglish
  "haa","ha","hae","hn","hnn","haan","achha","accha","acha","asha","thik","thike","thik ache",
  "thikache","thikase","thk","hmm","hm","hu","hum","obossoi","obosshoi","jani","ji","jee","jii",
  "nibo","nebo","korbo","dibo","hobe","hoi","raji",
  // Bangla
  "হ্যাঁ","হ্যা","হাঁ","হা","হুম","হম","জি","জ্বি","জ্বী","জী","আচ্ছা","ঠিক","ঠিক আছে","ঠিকাছে",
  "অবশ্যই","ওকে","কনফার্ম","করব","করবো","নিব","নিবো","নেব","নেবো","রাজি","হবে",
  // "show more" style affirmatives (used by the pager)
  "aro","aro dekhao","aro dekhabo","aro dekhte chai","more","show more","dekhao","dekhan","next",
  "আরও","আরো","আরও দেখান","আরও দেখতে চাই","আরও দেখাও","দেখাও","দেখান",
];
const NO = [
  // English
  "no","nope","nah","naw","never","dont","not","nvm","nope","noo","nooo",
  // Banglish
  "na","naa","naah","nahh","nai","nei","thak","lagbe na","dorkar nei","dorkar nai","na thak",
  "hobe na","bad dao","baad","byas","bas","enough","ar na","ar lagbe na","ar dorkar nei",
  // Bangla
  "না","নাা","নাহ","নাহহ","নাই","নেই","থাক","লাগবে না","দরকার নেই","দরকার নাই","হবে না",
  "আর না","আর লাগবে না","ব্যাস","যথেষ্ট","বাদ",
];

// Whole-word yes / no detection reused everywhere (confirm steps + pager).
const isYes = (text) => saysAny(text, YES);
const isNo = (text) => saysAny(text, NO);

// Fetch one page of products for a pager descriptor.
//   page = { mode: "search"|"brand"|"browse", query?, brand?, offset? }
// Returns { items, hasMore }. Fetches PAGE+1 to know if a next page exists.
async function fetchPage(page) {
  const base = { status: "published", deletedAt: null };
  const offset = page.offset || 0;
  if (page.mode === "brand") {
    const rows = await Product.find({ ...base, department: page.brand })
      .sort({ createdAt: -1 }).skip(offset).limit(PAGE + 1).lean();
  ...
  // search: re-run the relevance search big enough to cover this page + 1.
  const all = await searchProducts(page.query, offset + PAGE + 1);
  return { items: all.slice(offset, offset + PAGE), hasMore: all.length > offset + PAGE };
}

// Build a choose_product response: renders the freshly-fetched batch (numbered
// continuing from what's already shown), remembers the pager + all candidates
// so far, and — when more exist — asks whether to show more.
function buildListing({ intro = "", prevCandidates = [], newItems, page, hasMore }) {
  const start = prevCandidates.length;
  const batch = newItems.map((p) => ({
    productId: p._id.toString(),
    title: p.title,
    price: priceOf(p),
  }));
  const candidates = [...prevCandidates, ...batch];
  const lines = listLines(batch, start);
  const tail = hasMore
    ? `\n\nআরও পণ্য আছে — আপনি কি আরও দেখতে চান? "হ্যাঁ" / "না" লিখুন 🙂\n(অথবা নাম্বার লিখে পছন্দেরটা নিন, কিংবা অন্য নাম লিখুন)`
  ...
  return {
    ...
    state: {
      step: "choose_product",
      candidates,
      page: { ...page, offset: candidates.length },
      hasMore,
    },
  };
}
...
// List a brand's products with the total count (first page + "show more").
async function brandListing(brand) {
  ...
  const { items, hasMore } = await fetchPage({ mode: "brand", brand, offset: 0 });
  return buildListing({
    intro: `${label} ব্র্যান্ডে মোট ${total} টি পণ্য আছে 👇`,
    newItems: items,
    page: { mode: "brand", brand },
    hasMore,
  });
}
...
function reAsk(step, s) {
  ...
  if (step === "choose_product" && Array.isArray(s.candidates)) {
    const list = listLines(s.candidates);
    return `${list}\n\nকোনটা নেবেন? নাম্বার লিখুন (1, 2…) অথবা নাম লিখুন।`;
  }
  return "";
}
...
async function productIntent(text) {
  ...
  // "products dekhte chai" / "ki ki ache" — DON'T dump the whole catalog. Ask
  // which product they want to see first; the name they type next lands in
  // product search (via the `awaiting_product_name` step). Only a message that
  // ALSO carries a product word beyond the bare browse phrase falls through to
  // search below, so "router dekhte chai" still lists routers directly.
  if (has(text, KW.browse)) {
    const extra = keywords(text).filter((w) => !BROWSE_WORDS.has(w));
    if (extra.length === 0) {
      return { reply: pick(SAY.askProduct), state: { step: "awaiting_product_name" } };
    }
  }
  ...
  const { items: products, hasMore } = await fetchPage({ mode: "search", query: text, offset: 0 });
  if (products.length) {
    ...
    return buildListing({
      intro: "এই পণ্যগুলো পেলাম 👇",
      newItems: products,
      page: { mode: "search", query: text },
      hasMore,
    });
  }
...
async function _handle({ text, state, meta = {} }) {
  ...
  if (step === "choose_product") {
    // A number ("2") or a name in the current list picks that product — checked
    // first so digits are never mistaken for a "show more" yes/no.
    const chosen = resolvePick(text, s.candidates);
    if (chosen) {
      s = { step: "qty", items: [{ productId: chosen.productId, quantity: 1 }], _titles: { [chosen.productId]: chosen.title } };
      return { reply: `"${cleanTitle(chosen.title)}" — দাম ${money(chosen.price)}।\n${ASK.qty}`, state: s };
    }
    // At a "want to see more?" prompt: "না" → stop, "হ্যাঁ" → next page.
    // ("no" checked first — a negation can contain an affirmative word.)
    if (s.hasMore && s.page) {
      if (isNo(text)) {
        return {
          reply: `ঠিক আছে 🙂 উপরের লিস্ট থেকে নাম্বার লিখে পছন্দেরটা নিন, অথবা পণ্যের নাম লিখুন।`,
          state: { ...s, hasMore: false },
        };
      }
      if (isYes(text)) {
        const { items, hasMore } = await fetchPage(s.page);
        if (!items.length) {
          return {
            reply: `এই মুহূর্তে আর কোনো পণ্য নেই 🙂 উপরের লিস্ট থেকে নাম্বার লিখে পছন্দেরটা নিন, অথবা নাম লিখুন।`,
            state: { ...s, hasMore: false },
          };
        }
        return buildListing({
          intro: "আরও কিছু পণ্য 👇",
          prevCandidates: s.candidates,
          newItems: items,
          page: s.page,
          hasMore,
        });
  ...
  if (step === "confirm_single") {
    // Check "no" before "yes": a negation like "hobe na"/"korbo na" contains an
    // affirmative word, so yes-first would misread it.
    if (isNo(text)) {
      return {
        reply: "ঠিক আছে 🙂 অন্য কোনো পণ্য খুঁজলে নাম লিখুন, সাহায্য করছি।",
        state: {},
      };
    }
    if (isYes(text)) {
