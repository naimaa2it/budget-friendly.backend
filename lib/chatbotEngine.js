...
const KW = {
  greeting: ["hi","hello","hey","assalam","salam","আসসালাম","হ্যালো","hallo","nomoskar","kemon achen","how are you","good morning","good evening","শুভ সকাল"],
  thanks: [
    "thanks","thank you","thank u","thanku","thnx","tnx","tqsm","ty",
    "dhonnobad","onek dhonnobad","dhonnobaad","shukriya","ধন্যবাদ","শুকরিয়া","thx",
  ],
  agent: [
    ...
    "কথা বলবো","প্রতিনিধি","এজেন্ট","মানুষের সাথে","real human","talk to human",
  ],
  browse: [
    ...
    "ki ki ache","ki ki paoa jay","product gula","koto product","পণ্য দেখব","প্রোডাক্ট দেখতে",
    "কি কি আছে","কি কি পণ্য","সব পণ্য","পণ্যের তালিকা","browse",
  ],
...
async function _handle({ text, state, meta = {} }) {
  ...
  if (step === "choose_product") {
    ...
    return { reply: `বুঝতে পারিনি 🙈 — ${reAsk("choose_product", s)}`, state: s };
  }
  ...
  if (has(text, KW.thanks) && keywords(text).length <= 4) {
    return {
      reply: "আপনাকেও অসংখ্য ধন্যবাদ 🙏 আরও কিছু দরকার হলে নির্দ্বিধায় বলুন 🙂",
      state: {},
    };
  }
  ...
  if (has(text, KW.agent)) {
    const info = await storeInfo();
    return {
      reply: `ঠিক আছে 🙂 আমাদের একজন টিম মেম্বারকে জানিয়ে দিচ্ছি — একটু পরেই এখানে রিপ্লাই পাবেন।${info.phone ? " জরুরি হলে কল করুন: " + info.phone : ""}`,
      state: {},
      flag: true,
    };
  }

  // Support questions (track + trained Q&A) first…
  const side = await sideAnswer(text);
  if (side) return { reply: side, state: {} };
  ...
  const info = await storeInfo();
  return {
    reply: `ধন্যবাদ! 🙏 আপনার প্রশ্নটা আমাদের একজন টিম মেম্বারকে দিয়ে দিচ্ছি — শীঘ্রই এখানে রিপ্লাই পাবেন।${info.phone ? " অথবা কল করুন: " + info.phone : ""}`,
    state: {},
    flag: true,
  };
}
