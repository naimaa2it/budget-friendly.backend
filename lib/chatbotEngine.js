...
// chat bubble. `items` is an array of { title, price }.
function listLines(items) {
  return items
    .map((it, i) => `${i + 1}. ${cleanTitle(it.title)}\n    💵 ${money(it.price)}`)
    .join("\n\n");
}
...
async function brandListing(brand) {
  ...
  const lines = listLines(items.map((p) => ({ title: p.title, price: priceOf(p) })));
  const more =
    total > items.length
...
async function productIntent(text) {
  ...
  if (has(text, KW.browse)) {
    const featured = await Product.find({ status: "published", deletedAt: null })
      ...
      .lean();
    if (featured.length) {
      const list = listLines(featured.map((p) => ({ title: p.title, price: priceOf(p) })));
      return {
  ...
  if (products.length) {
    ...
    const list = listLines(products.map((p) => ({ title: p.title, price: priceOf(p) })));
