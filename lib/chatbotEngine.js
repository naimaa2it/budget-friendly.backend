...
async function _handle({ text, state, meta = {} }) {
  ...
  if (step === "choose_product") {
    ...
    // Otherwise the customer may have typed a NEW product / query — reset and
    // fall through to fresh idle handling instead of re-asking "which one?".
    s = { draft: {} };
  }
  ...
  if (step === "qty") {
    ...
    if (n == null) {
      // Not a number → maybe they asked something ("delivery charge koto?").
      const side = await sideAnswer(text);
      if (side) return { reply: `${side}\n\n${ASK.qty}`, state: s };
  ...
  if (products.length) {
    ...
    // that product directly — instead of dumping a generic 6-item list.
