...
const KW = {
  ...
  track: ["track","order koi","order kothay","parcel koi","track order","koi amar order"],
};
...
// ── main entry ───────────────────────────────────────────────────────────────
// Returns { reply, state, order?, flag? }
export async function handleChatMessage({ text, state, meta = {} }) {
  ...
  if (step === "choose_product") {
    ...
    if (!pick) {
      return {
        reply: "কোনটা নেবেন বলুন — নাম্বার লিখুন (1, 2, 3…) অথবা পণ্যের নাম লিখুন।",
        state: s,
      };
    }
  ...
  if (step === "address") {
    ...
    try {
      ...
      return { reply: `একটা সমস্যা হলো: ${e.message}। আবার চেষ্টা করুন অথবা আমাদের টিমের সাথে যোগাযোগ করুন।`, state: {}, flag: true };
    }
    s.step = "confirm";
