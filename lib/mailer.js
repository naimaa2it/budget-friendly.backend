...
async function getStoreName() {
  ...
  try {
    ...
    _storeNameCache = s?.storeName || process.env.STORE_NAME || "SmartBuy BD";
  } catch {
    _storeNameCache = process.env.STORE_NAME || "SmartBuy BD";
  }
...
function makeTransporter(user, pass) {
  if (!process.env.SMTP_HOST || !user || !pass) {
    console.warn(`[mailer] SMTP not configured — host=${process.env.SMTP_HOST} user=${user}`);
    return null;
  }
  ...
  t.verify().then(() => {
    console.log(`[mailer] SMTP OK: ${user} → ${process.env.SMTP_HOST}:${port}`);
  }).catch((err) => {
...
export async function sendOrderConfirmationEmail(order) {
  ...
  if (!toEmail) {
    console.warn(`[mailer] sendOrderConfirmationEmail skipped — no email on order ${order._id}`);
    return;
  }
...
export async function sendPaymentConfirmedEmail(order) {
  ...
  const toEmail = order.billingDetails?.email || order.userEmail;
  if (!toEmail) {
    console.warn(`[mailer] sendPaymentConfirmedEmail skipped — no email on order ${order._id}`);
    return;
  }
  const STORE_NAME = await getStoreName();
  const billing = order.billingDetails || {};
...
export async function sendOrderCancelledEmail(
  ...
  const toEmail = order.billingDetails?.email || order.userEmail;
  if (!toEmail) {
    console.warn(`[mailer] sendOrderCancelledEmail skipped — no email on order ${order._id}`);
    return;
  }
  const STORE_NAME = await getStoreName();
  const billing = order.billingDetails || {};
...
<table width="100%" cellpadding="0" cellspacing="0">
  ...
  results.forEach((r, i) => {
    if (r.status === "fulfilled") {
      ...
      console.error(`[mailer] Contact email FAILED → ${recipients[i]}:`, r.reason?.message);
    }
  });
}
