...
let _storePhoneCache = null;
let _storePhoneCacheTime = 0;
async function getStorePhone() {
  const now = Date.now();
  if (_storePhoneCache !== null && now - _storePhoneCacheTime < 60000)
    return _storePhoneCache;
  try {
    const Setting = (await import("../models/Setting.js")).default;
    const s = await Setting.findOne().lean();
    _storePhoneCache = s?.supportInfo?.phone || process.env.STORE_PHONE || "";
  } catch {
    _storePhoneCache = process.env.STORE_PHONE || "";
  }
  _storePhoneCacheTime = Date.now();
  return _storePhoneCache;
}
...
function makeTransporter(user, pass) {
  ...
  const t = nodemailer.createTransport({
    ...
    family: 4, // SMTP host resolves to an IPv6 address too; this network has no IPv6 route, causing ENETUNREACH
  });
...
// Generic admin alert — used by the automation engine (the "notify_admin"
// action) and by any channel that needs to ping the store owner. Never
// throws; safe to call fire-and-forget.
export async function sendAdminAlert({ subject, html, text }) {
  try {
    const t = getSupportTransporter() || getOrderTransporter();
    if (!t) {
      console.warn("[mailer] sendAdminAlert skipped — no SMTP transporter");
      return;
    }
    const storeName = await getStoreName();
    await t.sendMail({
      from: `"${storeName} Bot" <${SUPPORT_EMAIL || ORDER_EMAIL}>`,
      to: ADMIN_EMAIL,
      subject,
      ...(text ? { text } : {}),
      ...(html ? { html } : {}),
    ...
    console.error(`[mailer] sendAdminAlert failed: ${err.message}`);
  }
}

// Must match formatOrderIdSuffix() in lib/orderLookup.js so the ID in the
// email matches what's shown in the dashboard/user order routes.
...
const customerEmailHtml = (order, STORE_NAME, storePhone, opts = {}) => {
  const billing = order.billingDetails || {};
  const addr = [billing.address, billing.area, billing.zone, billing.city]
    .filter(Boolean)
    .join(", ");
  const orderNum = shortId(order._id);
  const headline = opts.headline || "Thanks for your Order!";
  const message =
    opts.message ||
    `Hi <strong>${billing.name || "Valued Customer"}</strong>, we&apos;re getting your order ready to be shipped.
          ...
          ${
            order.paymentMethod === "cash-on-delivery"
              ? "Your Cash on Delivery order will be confirmed after 60 minutes."
  ...
  <tr><td align="center" style="padding:30px 16px;">
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
      ...
      <tr><td style="padding:32px 32px 0;">
        <h2 style="margin:0 0 8px;font-size:20px;color:#1a1a2e;">${headline}</h2>
      ...
      <tr><td style="padding:0 32px 24px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #f0f0f0;border-radius:8px;overflow:hidden;">
          ...
          <tr><td style="padding:16px 18px;font-size:13px;color:#555;line-height:1.8;">
            ...
            ${billing.city === "Dhaka" || billing.city === "ঢাকা" ? "Inside Dhaka Delivery" : "Outside Delivery"}<br/><br/>
      ...
      <tr><td style="padding:20px 32px;background:#f9f9f9;border-top:1px solid #f0f0f0;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;color:#888;">
          ...
          ${storePhone ? ' | Phone: <span style="color:#1a1a2e;">' + storePhone + "</span>" : ""}
        </p>
...
const adminEmailHtml = (order, STORE_NAME, opts = {}) => {
  const billing = order.billingDetails || {};
  const orderNum = shortId(order._id);
  const heading = opts.heading || "🛒 New Order Received";
  const intro =
    opts.intro ||
    `A new ${order.paymentMethod} order has been placed on <strong>${STORE_NAME}</strong>.`;
...
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:10px;padding:32px;box-shadow:0 2px 8px rgba(0,0,0,.08);">
  <h2 style="color:#1a1a2e;margin-top:0;">${heading} — #${orderNum}</h2>
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;color:#333;margin-bottom:24px;">
    ...
    <tr><td style="padding:6px 0;border-bottom:1px solid #f0f0f0;"><strong>Address</strong></td><td style="padding:6px 0;border-bottom:1px solid #f0f0f0;">${[billing.address, billing.area, billing.zone, billing.city].filter(Boolean).join(", ") || "N/A"}</td></tr>
...
export async function sendOrderConfirmationEmail(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      to: toEmail,
      subject: `Order Received — #${shortId(order._id)} | ${STORE_NAME}`,
      html: customerEmailHtml(order, STORE_NAME, storePhone),
    });
    console.log(`[mailer] Order received email sent → ${toEmail}`);
  } catch (err) {
...
// ─── Order Confirmed Email (dashboard status → "confirmed") ─────────────────
  ...
  if (toEmail) {
    try {
      await transporter.sendMail({
        from: `"${STORE_NAME}" <${ORDER_EMAIL}>`,
        to: toEmail,
        subject: `Order Confirmed — #${orderNum} | ${STORE_NAME}`,
        html: customerEmailHtml(order, STORE_NAME, storePhone, {
          headline: "Your Order is Confirmed!",
          message: `Hi <strong>${order.billingDetails?.name || "Valued Customer"}</strong>, great news — your order has been confirmed and we&apos;re now processing it for shipment.`,
          policyNote: `Order will be delivered within 24 to 72 hours. You will receive another email once it has been shipped.`,
        }),
      });
      console.log(`[mailer] Order confirmed email sent → ${toEmail}`);
    } catch (err) {
      console.error(
        `[mailer] sendOrderConfirmedEmail (customer) failed:`,
        err.message,
      );
    }
  }
    ...
    try {
      await transporter.sendMail({
        ...
        html: adminEmailHtml(order, STORE_NAME, {
          ...
          intro: `Order #${orderNum} has been confirmed on <strong>${STORE_NAME}</strong>.`,
        }),
      });
      console.log(
        `[mailer] Order confirmed admin notification sent → ${ADMIN_EMAIL}`,
      );
    } catch (err) {
      console.error(
        `[mailer] sendOrderConfirmedEmail (admin) failed:`,
        err.message,
      );
    }
  }
    ...
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
      ...
      <tr><td style="padding:20px 32px;background:#f9f9f9;border-top:1px solid #f0f0f0;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;color:#888;">
          ...
          ${storePhone ? ' or call <span style="color:#1a1a2e;">' + storePhone + "</span>" : ""}
        </p>
