...
// Full human-facing order number token, including its prefix. New orders →
// "pk100000"; legacy orders (no orderNo) → "#<_id suffix>". Matches
// formatOrderNumber() in lib/orderLookup.js.
const orderLabel = (order) =>
  order && order.orderNo != null && order.orderNo !== ""
    ? `pk${order.orderNo}`
    : `${orderLabel(order)}`;
...
const customerEmailHtml = (order, STORE_NAME, storePhone, opts = {}) => {
  ...
  const addr = [billing.address, billing.area, billing.zone, billing.city]
    .filter(Boolean)
    .join(", ");
  const orderNum = orderLabel(order);
...
export async function sendOrderConfirmationEmail(order) {
  ...
  try {
    await transporter.sendMail({
      ...
      to: toEmail,
      subject: `Order Received — ${orderLabel(order)} | ${STORE_NAME}`,
