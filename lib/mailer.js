...
export async function sendAdminOrderNotification(order) {
  ...
  if (!transporter || !ADMIN_EMAIL) {
    console.warn('SMTP not configured or ADMIN_EMAIL missing — skipping admin email');
    return;
  }
  try {
    await transporter.sendMail({
...
export async function sendPaymentConfirmedEmail(order) {
  const transporter = getTransporter();
  if (!transporter) {
    console.warn('SMTP not configured — skipping payment confirmed email');
    return;
  }
  const toEmail = order.billingDetails?.email || order.userEmail;
  if (!toEmail) return;
...
<table width="100%" cellpadding="0" cellspacing="0">
  ...
  try {
    ...
    console.log(`Payment confirmed email sent to ${toEmail}`);
  } catch (err) {
    console.error('Payment confirmed email send error:', err.message);
  }
}
