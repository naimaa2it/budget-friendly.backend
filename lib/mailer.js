...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || STORE_EMAIL;
...
async function getStoreName() {
  const now = Date.now();
  if (_storeNameCache && now - _storeNameCacheTime < 60000) return _storeNameCache;
  try {
    const Setting = (await import('../models/Setting.js')).default;
    const s = await Setting.findOne().lean();
    _storeNameCache = s?.storeName || process.env.STORE_NAME || 'YourHaat';
  } catch {
    _storeNameCache = process.env.STORE_NAME || 'YourHaat';
  }
  _storeNameCacheTime = Date.now();
  return _storeNameCache;
}

// Create transporter lazily so missing SMTP config doesn't crash the server
...
const customerEmailHtml = (order, STORE_NAME) => {
  ...
  const addr = [billing.address, billing.zone, billing.city].filter(Boolean).join(', ');
    ...
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
      ...
      <tr><td style="padding:20px 32px;background:#f9f9f9;border-top:1px solid #f0f0f0;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;color:#888;">
          Email: <a href="mailto:${STORE_EMAIL}" style="color:#1a1a2e;">${STORE_EMAIL}</a>
...
const adminEmailHtml = (order, STORE_NAME) => {
  ...
  const itemList = (order.items || [])
    .map((i) => `<li style="margin-bottom:4px;">${i.title} × ${i.quantity} — ৳${((i.price || 0) * i.quantity).toFixed(2)}</li>`)
    .join('');
...
export async function sendAdminOrderNotification(order) {
  ...
  try {
    await transporter.sendMail({
      from: `"${STORE_NAME} Orders" <${STORE_EMAIL}>`,
      to: ADMIN_EMAIL,
    ...
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
      ...
      <tr><td style="padding:24px 32px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px;">
          ...
          ${order.valId ? `<tr><td colspan="2" style="padding-top:4px;font-size:12px;color:#888;">
...
export async function sendOrderCancelledEmail(order, { reason, cancelledBy = 'customer' } = {}) {
  const transporter = getTransporter();
  if (!transporter) return;
  const toEmail = order.billingDetails?.email || order.userEmail;
  if (!toEmail) return;
  ...
  const itemList = (order.items || [])
    .map((i) => `<li style="margin-bottom:4px;color:#555;font-size:13px;">${i.title} × ${i.quantity} — ৳${((i.price || 0) * i.quantity).toFixed(2)}</li>`)
    .join('');
    ...
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08);">
      ...
      <tr><td style="padding:0 32px 20px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:16px;">
          ...
          ${reason ? `<tr><td colspan="2" style="padding-top:8px;font-size:13px;color:#555;">
            Reason: <em>${reason}</em>
          </td></tr>` : ''}
        </table>
      </td></tr>
      ...
      <tr><td style="padding:20px 32px;background:#f9f9f9;border-top:1px solid #f0f0f0;text-align:center;">
        <p style="margin:0 0 4px;font-size:12px;color:#888;">
          Questions? Email us at <a href="mailto:${STORE_EMAIL}" style="color:#1a1a2e;">${STORE_EMAIL}</a>
