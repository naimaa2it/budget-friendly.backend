...
import { fetchCourierTracking } from './couriers/index.js';
...
export async function syncOrderShipment(orderDoc, { force = false } = {}) {
  ...
  // Courier tracking text is stored in order.shipment.courierStatus for display only.
  // order.status is admin/moderator-controlled and is never auto-changed from courier sync.
  if (
