 ...
 *
 * Env (reuse shipping creds where possible):
 *   PATHAO_USERNAME, PATHAO_PASSWORD — Pathao merchant panel login
 *   STEADFAST_EMAIL, STEADFAST_PASSWORD — Steadfast merchant panel
 *   REDX_PHONE, REDX_PASSWORD — RedX merchant panel
 */
...
let redxAccessToken = null;
let redxAccessTokenAt = 0;
...
function courierConfigured(name) {
  if (name === 'pathao') {
    return Boolean(process.env.PATHAO_USERNAME && process.env.PATHAO_PASSWORD);
  }
  if (name === 'steadfast') {
    return Boolean(process.env.STEADFAST_EMAIL && process.env.STEADFAST_PASSWORD);
  }
  if (name === 'redx') {
    return Boolean(process.env.REDX_PHONE && process.env.REDX_PASSWORD);
  }
  return false;
}
...
async function getPathaoMerchantToken() {
  if (pathaoMerchantToken && Date.now() - pathaoMerchantTokenAt < 50 * 60 * 1000) {
...
async function fetchPathaoLifetimeStats(phone) {
  if (!courierConfigured('pathao')) {
    return emptyCourierResult();
  }

  try {
    const token = await getPathaoMerchantToken();
    ...
    const delivered = Number(data?.data?.customer?.successful_delivery || 0);
    const total = Number(data?.data?.customer?.total_delivery || 0);
...
async function fetchSteadfastLifetimeStats(phone) {
  if (!courierConfigured('steadfast')) {
    return emptyCourierResult();
  }
...
async function getRedxAccessToken() {
  if (redxAccessToken && Date.now() - redxAccessTokenAt < 50 * 60 * 1000) {
    return redxAccessToken;
  }

  const phone = normalizeBdMobile(process.env.REDX_PHONE);
  ...
  redxAccessToken = token;
  redxAccessTokenAt = Date.now();
  return redxAccessToken;
}

async function fetchRedxLifetimeStats(phone) {
  if (!courierConfigured('redx')) {
    return emptyCourierResult();
  }

  try {
    const token = await getRedxAccessToken();
...
export function getLifetimeConfiguredCouriers() {
  return {
    pathao: courierConfigured('pathao'),
    steadfast: courierConfigured('steadfast'),
    redx: courierConfigured('redx'),
  };
}
...
export async function fetchLifetimeCourierStats(rawPhone, { skipCache = false } = {}) {
  ...
  if (!isValidBdMobile(phone)) {
    return {
      ...
      configured: getLifetimeConfiguredCouriers(),
  ...
  const configured = getLifetimeConfiguredCouriers();
