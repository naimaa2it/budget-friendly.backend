 ...
 * (Pathao, Steadfast, RedX). Not limited to SmartBuy BD orders.
...
const LIFETIME_CACHE_TTL_MS = Number(
  process.env.LIFETIME_STATS_CACHE_TTL_MS || 10 * 60 * 1000,
);
...
function fraudCredsReady(slug, creds) {
  ...
  if (s === "redx")
    return Boolean((creds.phone && creds.password) || creds.apiToken);
  return false;
}
...
async function isFraudConfigured(slug) {
  ...
  return (
    integration.fraudConfigured || fraudCredsReady(slug, integration.creds)
  );
}
...
async function fetchPathaoLifetimeStats(phone) {
  ...
  try {
    ...
    // Pathao v2 API — returns customer_rating instead of delivery counts
    if (payload.version === "v2" || payload.customer_rating) {
      const rating = String(payload.customer_rating || "unknown");
      const addressCount = Array.isArray(payload.address_book)
        ? payload.address_book.length
        : 0;
      // Map rating to approximate successRate for the gauge
      const ratingSuccessMap = {
        new_customer: null, // no history yet
        good_customer: 85,
        regular_customer: 65,
        bad_customer: 20,
        blocked: 0,
      };
      const successRate = ratingSuccessMap[rating] ?? null;
      return {
        delivered: 0,
        cancelled: 0,
        total: 0,
    ...
    // Legacy v1 response — numeric counts
...
async function fetchSteadfastLifetimeStats(phone) {
  ...
  if (creds.apiKey && creds.secretKey) {
    try {
      const { steadfastFraudCheck } =
        await import("./couriers/createSteadfastOrder.js");
      const result = await steadfastFraudCheck(
        creds,
  ...
  try {
    ...
    if (!csrfToken) {
      return buildCourierResult({
        ...
        total: 0,
        configured: true,
        error: "Steadfast login page changed — CSRF token not found",
      });
    }
    ...
    if (loginRes.status !== 302) {
      return buildCourierResult({
        ...
        total: 0,
        configured: true,
        error: "Steadfast login failed — check email/password",
      });
    }
  ...
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      return buildCourierResult({
        ...
        total: 0,
        configured: true,
        error: "RedX fraud check — merchant account access required",
      });
    }
    ...
    if (!res.ok) {
      return buildCourierResult({
        ...
        configured: true,
        error:
          data?.message || `RedX customer lookup failed (HTTP ${res.status})`,
      });
    }
...
export function aggregateLifetimeStats(couriers = {}) {
  ...
  if (totalParcels > 0) {
    ...
    if (cancellationRate >= 30) {
      riskFactors.push(
        `High cancellation rate across couriers (${cancellationRate}%)`,
      );
    }
    if (deliverySuccessRate < 50 && completed >= 3) {
      riskFactors.push(
        `Low lifetime delivery success (${deliverySuccessRate}%)`,
      );
    }
...
export async function fetchLifetimeCourierStats(
  rawPhone,
  { skipCache = false } = {},
) {
  ...
  const result = {
    ...
    description:
      "Lifetime parcel history for this mobile on Pathao, Steadfast, and RedX networks (all shops, not only SmartBuy BD).",
    configured,
    anyConfigured,
    couriers,
