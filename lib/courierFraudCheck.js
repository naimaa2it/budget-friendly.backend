...
async function fetchSteadfastLifetimeStats(phone) {
  ...
  if (creds.apiKey && creds.secretKey) {
    try {
      ...
      return buildCourierResult({
        delivered: 0, cancelled: 0, total: 0, configured: true,
  ...
  try {
    ...
    return buildCourierResult({ delivered, cancelled, total, configured: true });
  } catch (err) {
    return buildCourierResult({
      delivered: 0, cancelled: 0, total: 0, configured: true,
...
async function fetchRedxLifetimeStats(phone) {
  ...
  try {
    ...
    console.log('[RedX fraud check] token obtained, calling API for phone:', phone);
    ...
    const data = await res.json().catch(() => ({}));
    console.log('[RedX fraud check] status:', res.status, 'raw:', JSON.stringify(data));
    ...
    if (!res.ok) {
      return buildCourierResult({
        ...
        total: 0,
        configured: true,
        error: data?.message || `RedX customer lookup failed (HTTP ${res.status})`,
      });
    }
    ...
    const cancelled = Math.max(0, total - delivered);
    console.log('[RedX fraud check] parsed → delivered:', delivered, 'total:', total);
    ...
    console.log('[RedX fraud check] ERROR — phone:', phone, 'error:', err.message);
    return buildCourierResult({
      delivered: 0,
      cancelled: 0,
      total: 0,
