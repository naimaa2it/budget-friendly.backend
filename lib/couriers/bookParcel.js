...
export async function bookParcelWithCourier(order, courierSlug, options = {}) {
  ...
  // Product list, prefixed with the order id so it's plainly visible right
  // next to the item info on the courier panel (the order id also goes in the
  // dedicated merchant-invoice field below).
  const products = buildItemDescription(order);
  const itemDescription = products
    ? `#${merchantOrderId} — ${products}`.slice(0, 240)
    : `#${merchantOrderId}`;
