...
import {
  extractSteadfastEventsFromMerchantData,
  fetchSteadfastPublicTracking,
} from './steadfastPublicTracking.js';

async function fetchSteadfastMerchantTracking(trackingId) {
  ...
  let events = extractSteadfastEventsFromMerchantData(data);
  if (!events.length && status) {
    events = [
      {
        status: String(status),
        message: String(status),
        at: data?.updated_at ? new Date(data.updated_at) : new Date(),
        source: 'courier',
      },
    ];
  }
  ...
  return {
    configured: true,
    courierStatus: status ? String(status) : events[events.length - 1]?.message || null,
    events,
    raw: data,
  };
}
...
async function fetchSteadfastTracking(trackingId) {
  const pub = await fetchSteadfastPublicTracking(trackingId);
  if (pub.events?.length) {
    return pub;
  }

  try {
    const merchant = await fetchSteadfastMerchantTracking(trackingId);
    if (merchant.configured && (merchant.events?.length || merchant.courierStatus)) {
      return merchant;
    }
  } catch {
    // fall through to public partial result
  }
