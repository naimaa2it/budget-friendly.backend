...
function steadfastHttpError(text, status, data = null) {
  ...
  if (isSteadfastAccountInactive(combined)) {
    return (
      ...
      'steadfast.com.bd merchant panel এ login করে account status দেখুন, অথবা Steadfast support (09678-045045) এ যোগাযোগ করুন।'
    );
  }
...
function parseSteadfastError(data, statusCode) {
  ...
  const message = parts.filter(Boolean).join(' — ') || `Steadfast order failed (HTTP ${statusCode})`;
