...
function baseUrl(storeConfig, creds) {
  const raw = storeConfig.steadfastBaseUrl || creds.baseUrl || STEADFAST_API_BASE;
...
function steadfastResponseError(text, status) {
  const snippet = text.trim().slice(0, 200).replace(/\s+/g, ' ');
  if (status === 401) {
    return `Steadfast authentication failed (HTTP 401). API Key/Secret Key ভুল বা পুরনো — Shipment Settings এ আবার save করুন এবং Test connection চালান।${snippet ? ` (${snippet})` : ''}`;
  }
  return `Steadfast returned non-JSON (HTTP ${status}).${snippet ? ` ${snippet}` : ''}`;
}
...
function parseSteadfastError(data, statusCode) {
  ...
  if (Array.isArray(data.errors)) {
    parts.push(data.errors.map((e) => (typeof e === 'string' ? e : JSON.stringify(e))).join('; '));
  ...
  return parts.filter(Boolean).join(' — ') || `Steadfast order failed (HTTP ${statusCode})`;
}
...
export async function createSteadfastOrder(creds, storeConfig, payload) {
  ...
  const body = {
    ...
    recipient_phone: String(payload.recipientPhone || '').replace(/\D/g, '').slice(-11),
  ...
  if (!auth['Api-Key'] || !auth['Secret-Key']) {
    throw new Error('Steadfast API Key and Secret Key are missing. Save them in Shipment Settings.');
  }
  ...
  try {
    ...
    throw new Error(steadfastResponseError(text, res.status));
  }
