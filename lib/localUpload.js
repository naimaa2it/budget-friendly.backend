...
export function saveLocalImage(buffer, folder, ext = "webp") {
  ...
  return { public_id, url: `${PUBLIC_BASE_URL}/uploads/${public_id}`, filePath };
}
...
export async function processAndSaveImage(buffer, mimetype, folder) {
  ...
  const optimizedBuffer = await pipeline.clone().webp({ quality }).toBuffer();
  const meta = await sharp(optimizedBuffer).metadata();
  const { public_id, url, filePath } = saveLocalImage(optimizedBuffer, folder, "webp");

  // Old-browser fallback, mirrors Cloudinary's f_auto: save a same-named
  // .jpg next to the .webp. index.js's /uploads middleware serves this
  // instead when a client's Accept header doesn't advertise webp support.
  try {
    const jpegBuffer = await pipeline
      ...
      .jpeg({ quality })
      .toBuffer();
    fs.writeFileSync(filePath.replace(/\.webp$/, ".jpg"), jpegBuffer);
  } catch {
    // non-fatal — fallback simply won't be available for this asset
  }
...
export function deleteLocalImage(publicId) {
  ...
  fs.unlink(filePath, () => {});
  // .webp assets may have a same-named .jpg fallback (see processAndSaveImage)
  if (/\.webp$/i.test(filePath)) {
    fs.unlink(filePath.replace(/\.webp$/i, ".jpg"), () => {});
  }
}
  ...
  const walk = (dir, relPrefix) => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        // Skip auto-generated old-browser .jpg fallbacks (see
        // processAndSaveImage) — they're not standalone uploads.
        if (
          /\.jpg$/i.test(entry.name) &&
          names.has(entry.name.replace(/\.jpg$/i, ".webp"))
        ) {
          continue;
        }
        relPaths.push(rel);
